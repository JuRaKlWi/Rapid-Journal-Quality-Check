...
#b <- c(1:20)
#n <- substr(colnames(Names), 1, 6)
#Names.FullRank.txt <- c("Names.FullRank = \" ", apply(Names, 1, function(x) 
#  paste( paste( trimws(x), "-", n, "-", collapse="", sep=""), "X_X") ), " \" ")
#
#fileConn<-file("Names.FullRank.txt")
#writeLines(Names.FullRank.txt, fileConn, sep="\n")
#close(fileConn)
...
SJR <- as.data.frame( read.table("C:/Users/wmb500/sciebo/Github/Ranking Data/scimagojr 2021.csv", header=T, sep=";", dec=".") )
...
SJR.red <- SJR [,c("Title", "Issn_offline", "Issn_online", "SJR.Best.Quartile", "H.index")]


VHB <- as.data.frame( read_excel("C:/Users/wmb500/sciebo/Github/Ranking Data/VHB_3.xlsx") )
VHB$Issn <- gsub("-", "", VHB$ISSN)
VHB$Issn <- gsub("ÿ", "", VHB$Issn)

VHB.red <- VHB [,c("Zeitschriften", "JQ3", "Issn")]


dta1 <- merge(SJR.red, VHB.red, by.x="Issn_offline", by.y="Issn")

dta2 <- merge(SJR.red, VHB.red, by.x="Issn_online", by.y="Issn")

dta <- rbind.fill(dta1, dta2, 
                  SJR.red [ !((SJR.red$Title %in% dta1$Title) | (SJR.red$Title %in% dta2$Title)), ],
                  VHB.red [ !((VHB.red$Zeitschriften %in% dta1$Zeitschriften) | (VHB.red$Zeitschriften %in% dta2$Zeitschriften)), ])
